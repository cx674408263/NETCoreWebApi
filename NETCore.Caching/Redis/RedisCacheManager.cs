   ...
   public class RedisCacheManager :IRedisCacheManager
    {
        ...
        public async Task ClearAsync()
        {
            foreach (var endPoint in this.GetRedisConnection().GetEndPoints())
            {
                ...
                {
                    await redisConnection.GetDatabase(this.redisDatabase).KeyDeleteAsync(key);
                }
            }
        }
        ...
        {
            return await redisConnection.GetDatabase(this.redisDatabase).KeyExistsAsync(key);
        }
        ...
        {
            var value = await redisConnection.GetDatabase(this.redisDatabase).StringGetAsync(key);
            if (value.HasValue)
            {
        ...
        public async Task RemoveByKey(string key)
        {
            ...
            if (!redisResult.IsNull)
            {
                ...
                foreach (var k in keys)
                    redisConnection.GetDatabase(this.redisDatabase).KeyDelete(k);
        ...
        public async Task SetAsync(string key, object value, TimeSpan cacheTime)
        {
            if (value != null)
            {
                ...
                await redisConnection.GetDatabase(this.redisDatabase).StringSetAsync(key, SerializeHelper.Serialize(value), cacheTime);
            }
        }

        public async Task<bool> SetValueAsync(string key, byte[] value)
        {
            return await redisConnection.GetDatabase(this.redisDatabase).StringSetAsync(key, value, TimeSpan.FromSeconds(120));
        }
